...
public class QuestSystem : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (!Load())
        {
            foreach (var ahcievement in achievementDatabase.Quests)
            {
                Register(ahcievement);
            }
        }
    }
    ...
    private bool Load()
    {
        if (PlayerPrefs.HasKey(kSaveRootPath))
        {
            ...
            LoadSaveData(root[kActiveQuestsSavePath], questDatabase, LoadActiveQuest);
            LoadSaveData(root[kCompletedQuestsSavePath], questDatabase, LoadCompletedQuest);

            LoadSaveData(root[kActiveAchievementsSavePath], achievementDatabase, LoadActiveQuest);
            LoadSaveData(root[kCompletedAchievementsSavePath], achievementDatabase, LoadCompletedQuest);
            return true;
        }
        else
            return false;
